...
def main() -> None:
    ...
    def maybe_start_post_display_spatial_dwell(agent_id: int) -> bool:
        # SC degraded run: disabled degraded-only spatial dwell behavior.
        return False
        ...
        for _ in range(STEPS_PER_FRAME * SIM_SKIP):
            ...
            for s in range(num_security):
                ...
                if security_queue_front_wait_remaining[s] <= 0:
                    ...
                    security_service_remaining[s] = get_current_security_service_time(
                        sim_time_now
                    )
                    sim.switch_agent_journey(
    ...
    scenario_prefix = "3_SC_degarded_"

    def prefixed_name(filename: str) -> str:
        return f"{scenario_prefix}{filename}"

    metrics_path = os.path.abspath(os.path.join(_out_dir, prefixed_name("sc_degarded_metrics.json")))
    timeseries_json_path = os.path.abspath(os.path.join(_out_dir, prefixed_name("sc_degarded_timeseries.json")))
    timeseries_csv_path = os.path.abspath(os.path.join(_out_dir, prefixed_name("sc_degarded_timeseries.csv")))
    plotdata_json_path = os.path.abspath(os.path.join(_out_dir, prefixed_name("sc_degarded_plotdata.json")))
    ...
    try:
        ...
        if time_steps and exited_counts and len(time_steps) == len(exited_counts):
            ...
            if t_h_plot:
                ...
                ax1.set_title("Passenger Throughput Over Time (Security Degarded)")
                fig1.tight_layout()
        ...
        if (
            ...
            ax2.set_title("Security Degarded Queue Lengths vs Time")
            ax2.legend(loc="best")
            fig2.tight_layout()
        ...
        if time_steps and exited_counts and len(time_steps) == len(exited_counts):
            ...
            ax3.set_title("Security Degarded Cumulative Exited Passengers vs Time")
            ax3.legend(loc="best")
            fig3.tight_layout()
        ...
        if zone_sample_steps > 0:
            ...
            ax4.set_title(
                "Spatial Congestion Heatmap Over Airport Layout - Security Degarded Scenario",
                fontsize=16,
                color="white",
            )
    ...
    print(
        ...
        f"(check-in {NORMAL_CI_SERVICE_TIME:.0f}s throughout; "
        f"security {NORMAL_SECURITY_SERVICE_TIME:.0f}s before "
        f"{DEGRADATION_START_TIME:.0f}s, "
        f"{DEGRADED_SECURITY_SERVICE_TIME:.0f}s after)",
        flush=True,
    )
