...
def main() -> None:
    ...
    def maybe_start_post_display_spatial_dwell(agent_id: int) -> bool:
        # Baseline run: disabled degraded-only spatial dwell behavior.
        return False
    ...
    # Time-series logs for baseline analysis
    ...
    # Zone-based occupancy aggregation for baseline hotspot (same logic class as scenario heatmaps).
    ZONE_COLS = 50
    ZONE_ROWS = 30
            ...
            for j in range(num_counters):
                ...
                if dist >= COUNTER_DIST_AT_FRONT:
                    ...
                    if counter_queue_front_wait_remaining[j] <= 0:
                        ...
                        counter_service_remaining[j] = NORMAL_CI_SERVICE_TIME
                        sim.switch_agent_journey(
                            front_id,
    ...
    baseline_prefix = "1_Baseline_"

    def prefixed_name(filename: str) -> str:
        return f"{baseline_prefix}{filename}"

    metrics_path = os.path.abspath(os.path.join(_out_dir, prefixed_name("baseline_metrics.json")))
    timeseries_json_path = os.path.abspath(os.path.join(_out_dir, prefixed_name("baseline_timeseries.json")))
    timeseries_csv_path = os.path.abspath(os.path.join(_out_dir, prefixed_name("baseline_timeseries.csv")))
    plotdata_json_path = os.path.abspath(os.path.join(_out_dir, prefixed_name("baseline_plotdata.json")))
    ...
    try:
        ...
        if time_steps and exited_counts and len(time_steps) == len(exited_counts):
            ...
            if t_h_plot:
                ...
                ax1.set_title("Passenger Throughput Over Time (Baseline)")
                fig1.tight_layout()
        ...
        if (
            ...
            ax2.set_title("Baseline Queue Lengths vs Time")
            ax2.legend(loc="best")
            fig2.tight_layout()
        ...
        if time_steps and exited_counts and len(time_steps) == len(exited_counts):
            ...
            ax3.set_title("Baseline Cumulative Exited Passengers vs Time")
            ax3.legend(loc="best")
            fig3.tight_layout()
        ...
        if zone_sample_steps > 0:
            ...
            cbar_overlay.set_label(
                "Average Passenger Occupancy", color="white", fontsize=14
            )
            cbar_overlay.ax.yaxis.set_tick_params(color="white", labelsize=12)
            plt.setp(
                cbar_overlay.ax.get_yticklabels(), color="white", fontsize=12
            )
            ...
            ax4.set_xlabel("Airport Width Position (m)", color="white", fontsize=14)
            ax4.set_ylabel("Airport Length Position (m)", color="white", fontsize=14)
            ...
            ax4.set_title(
                "Spatial Congestion Heatmap Over Airport Layout - Baseline Scenario",
                fontsize=16,
                color="white",
            )
            fig4.subplots_adjust(left=0.10, right=0.88, bottom=0.12, top=0.90)
    ...
    print(f"Running {scenario} scenario (no disruptions)")
