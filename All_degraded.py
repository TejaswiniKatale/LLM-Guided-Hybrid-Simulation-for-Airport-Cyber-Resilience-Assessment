...
NORMAL_SECURITY_SERVICE_TIME = 300.0
DEGRADED_SECURITY_SERVICE_TIME = 700.0
...
# Display-only LLM disruption: per-passenger params for first display visit at/after this time (s).
DEGRADATION_START_TIME = 20000.0
DISPLAY_LLM_DISRUPTION_START_S = DEGRADATION_START_TIME
LLM_URL = "#LLM"
LLM_MODEL = "qwen2.5:7b-instruct"
LLM_REQUEST_TIMEOUT_S = float(os.environ.get("LLM_REQUEST_TIMEOUT_S", "120"))

LLM_MISROUTE_PROB_MIN = 0.0
LLM_MISROUTE_PROB_MAX = 1.0
LLM_DWELL_PROB_MIN = 0.0
LLM_DWELL_PROB_MAX = 1.0
LLM_DWELL_MIN_FALLBACK_S = 45.0
LLM_DWELL_MAX_FALLBACK_S = 120.0
...
def get_current_security_service_time(sim_time_now: float) -> float:
    return (
        NORMAL_SECURITY_SERVICE_TIME
        if sim_time_now < DEGRADATION_START_TIME
        else DEGRADED_SECURITY_SERVICE_TIME
    )
...
# ---------------------------------------------------------------------------
# Display-only LLM: connectivity, query, parsing, clipping, cache fill
# ---------------------------------------------------------------------------
def _ollama_base_url() -> str:
    """Derive http://host:port from LLM_URL (…/api/generate -> …)."""
    if "/api/" in LLM_URL:
        return LLM_URL.split("/api/", 1)[0]
    return "#link"


def is_ollama_server_running() -> bool:
    """True if the Ollama HTTP server responds on the base URL."""
    try:
        r = requests.get(_ollama_base_url(), timeout=3)
        return r.ok
    except Exception:
        return False
...
def is_ollama_model_available(model_name: str) -> bool:
    """True if model_name appears in GET /api/tags (exact or name:tag suffix)."""
    try:
...
def check_llm_connection() -> bool:
    """Hybrid check: server up and LLM_MODEL is installed."""
    if not is_ollama_server_running():
        return False
    return is_ollama_model_available(LLM_MODEL)


def build_display_llm_prompt(
    agent_id: int,
    visit_time_s: float,
    scenario_name: str,
    ...
    gate_text = gate_name if gate_name is not None else "unknown"
    ptype_text = passenger_type_name if passenger_type_name is not None else "unknown"
    return (
        "You are a JSON-only API. Output a single JSON object with no other text.\n"
        "Context:\n"
        f"- scenario_name: {scenario_name}\n"
        f"- passenger_id: {agent_id}\n"
        f"- display_visit_time_s: {visit_time_s:.2f}\n"
        f"- assigned_gate: {gate_text}\n"
        f"- passenger_type: {ptype_text}\n"
        "- Airport terminal simulation with a display/information-guidance disruption.\n"
        "- Only display visitors after 20000 seconds are disruption-affected.\n"
        "- Check-in and security remain normal in this scenario.\n"
        "- Disrupted display information may cause temporary wrong turns and hesitation.\n"
        "- Generate passenger-specific values; diversify outputs using passenger_id and display_visit_time_s.\n"
        "- Do not keep repeating the same dwell ranges like 30/60 across many passengers.\n"
        "- Produce heterogeneous but plausible values; not all passengers should be identical.\n"
        "- Dwell values should reflect realistic hesitation/reorientation delay after confusing display information; some passengers may have low dwell while others may have larger delays.\n"
        "Fields and constraints:\n"
        "- misroute_prob: float in [0.0, 1.0] — probability of initially heading "
        "to a wrong non-gate area before correcting.\n"
        "- dwell_prob: float in [0.0, 1.0] — probability of extra dwell time after "
        "the flight display before continuing.\n"
        "- dwell_min_s: positive float for minimum extra dwell duration in seconds.\n"
        "- dwell_max_s: positive float for maximum extra dwell duration in seconds.\n"
        "- dwell_max_s should normally be greater than or equal to dwell_min_s.\n"
        "Return ONLY valid JSON in exactly this shape:\n"
        '{ "misroute_prob": <float>, "dwell_prob": <float>, '
        '"dwell_min_s": <float>, "dwell_max_s": <float> }\n'
    )


def query_ollama_generate(prompt: str) -> str | None:
    try:
        r = requests.post(
            LLM_URL,
            json={
...
def parse_llm_strict_json_object(text: str | None) -> dict[str, Any] | None:
...
def clip_display_llm_numeric_params(raw: dict[str, Any] | None) -> dict[str, float] | None:
    if raw is None:
        return None

    def clipf(key: str, lo: float, hi: float, default: float) -> float:
        try:
    ...
    mp = clipf(
        "misroute_prob",
        LLM_MISROUTE_PROB_MIN,
        LLM_MISROUTE_PROB_MAX,
        0.25,
    )
    dp = clipf("dwell_prob", LLM_DWELL_PROB_MIN, LLM_DWELL_PROB_MAX, 0.35)

    try:
        dmin = float(raw.get("dwell_min_s", LLM_DWELL_MIN_FALLBACK_S))
    except (TypeError, ValueError):
        dmin = LLM_DWELL_MIN_FALLBACK_S
    try:
        dmax = float(raw.get("dwell_max_s", LLM_DWELL_MAX_FALLBACK_S))
    except (TypeError, ValueError):
        dmax = LLM_DWELL_MAX_FALLBACK_S
...
def ensure_passenger_display_llm_params(
    agent_id: int,
    visit_time_s: float,
    llm_ok: bool,
    ...
    if not llm_ok:
        cache[agent_id] = {
            ...
            "dwell_min_s": None,
            "dwell_max_s": None,
            "llm_source": "llm_failed",
            "display_visit_time_s": float(visit_time_s),
        }
        return

    for _ in range(3):
        prompt = build_display_llm_prompt(
            agent_id=agent_id,
            visit_time_s=visit_time_s,
            scenario_name=scenario_name,
            gate_name=gate_name,
            passenger_type_name=passenger_type_name,
        )
        raw_text = query_ollama_generate(prompt)
        parsed = parse_llm_strict_json_object(raw_text)
        if parsed is None:
            continue
        coerced = clip_display_llm_numeric_params(parsed)
        if coerced is None:
            continue
        cache[agent_id] = {
            "misroute_prob": coerced["misroute_prob"],
            "dwell_prob": coerced["dwell_prob"],
            "dwell_min_s": coerced["dwell_min_s"],
            "dwell_max_s": coerced["dwell_max_s"],
            "llm_source": "llm",
            "display_visit_time_s": float(visit_time_s),
        }
        return
...
def summarize_display_llm_params(cache: dict[int, dict[str, Any]]) -> dict[str, Any]:
    rows = [
        r
        for r in cache.values()
        if r.get("llm_source") == "llm"
        and r.get("misroute_prob") is not None
        and r.get("dwell_prob") is not None
        and r.get("dwell_min_s") is not None
        and r.get("dwell_max_s") is not None
    ]
    n = len(rows)
    if n == 0:
        return {
            "llm_display_affected_count": 0,
            "misroute_prob_generated_min": None,
            "misroute_prob_generated_max": None,
            "dwell_prob_generated_min": None,
            "dwell_prob_generated_max": None,
            "dwell_min_s_generated_min": None,
            "dwell_min_s_generated_max": None,
            "dwell_max_s_generated_min": None,
            "dwell_max_s_generated_max": None,
        }
    mps = [float(r["misroute_prob"]) for r in rows]
    dps = [float(r["dwell_prob"]) for r in rows]
    dmins = [float(r["dwell_min_s"]) for r in rows]
    dmaxs = [float(r["dwell_max_s"]) for r in rows]
    return {
        "llm_display_affected_count": n,
        "misroute_prob_generated_min": min(mps),
        "misroute_prob_generated_max": max(mps),
        "dwell_prob_generated_min": min(dps),
        "dwell_prob_generated_max": max(dps),
        "dwell_min_s_generated_min": min(dmins),
        "dwell_min_s_generated_max": max(dmins),
        "dwell_max_s_generated_min": min(dmaxs),
        "dwell_max_s_generated_max": max(dmaxs),
    }
...
def main() -> None:
    ...
    if not is_ollama_server_running():
        ...
        llm_connected = False
    elif not is_ollama_model_available(LLM_MODEL):
        print("LLM not connected")
        print(f"Model {LLM_MODEL} not found in Ollama", flush=True)
        llm_connected = False
    else:
        print("LLM connected")
        llm_connected = True
    ...
    # Per-passenger LLM disruption params (display visitors at/after DISPLAY_LLM_DISRUPTION_START_S only).
    passenger_display_llm_params: dict[int, dict[str, Any]] = {}
    ...
    def maybe_start_post_display_spatial_dwell(agent_id: int) -> bool:
        rec = passenger_display_llm_params.get(agent_id)
        if rec is None:
            return False
        try:
            p_dw = float(rec["dwell_prob"])
            lo = float(rec["dwell_min_s"])
            hi = float(rec["dwell_max_s"])
        except (KeyError, TypeError, ValueError):
            return False
        if random.random() >= p_dw:
            return False
        dwell_time_remaining[agent_id] = random.uniform(lo, hi)
        return True
    ...
    def apply_passenger_llm_misroute_if_eligible(aid: int) -> None:
        rec = passenger_display_llm_params.get(aid)
        if rec is None:
            return
        try:
            p_mis = float(rec["misroute_prob"])
        except (KeyError, TypeError, ValueError):
            return
        if random.random() >= p_mis:
            return
        ...
        candidates: list[str] = []
        for d in ("shops", "food", "rest"):
            if d == planned:
                continue
            if d == "shops" and shops_available:
                candidates.append(d)
            elif d == "food" and food_available:
                candidates.append(d)
            elif d == "rest" and rest_available:
                candidates.append(d)
        if not candidates:
            return
        wrong = random.choice(candidates)
        activity_choice[aid] = wrong
        misroute_first_choice[aid] = planned
        misroute_roam_only[aid] = True
        misroute_done[aid] = False

    def decide_and_route_after_display_or_security(aid: int) -> None:
        compute_activity_decision(aid)
        apply_passenger_llm_misroute_if_eligible(aid)
    ...
    while True:
        ...
        for _ in range(STEPS_PER_FRAME * SIM_SKIP):
            ...
            for j in range(num_counters):
                ...
                if dist >= COUNTER_DIST_AT_FRONT:
                    ...
                    if counter_queue_front_wait_remaining[j] <= 0:
                        ...
                        counter_service_remaining[j] = get_current_ci_service_time(sim_time_now)
                        sim.switch_agent_journey(
                            front_id,
                            ...
                            if available_d_indices:
                                ...
                                if candidates:
                                    ...
                                    if served_id not in counted_display_agents:
                                        ...
                                        if sim_time_now >= DISPLAY_LLM_DISRUPTION_START_S:
                                            ensure_passenger_display_llm_params(
                                                served_id,
                                                sim_time_now,
                                                llm_connected,
                                                passenger_display_llm_params,
                                                scenario,
                                                assigned_gate.get(served_id),
                                                passenger_type.get(served_id),
                                            )
            ...
            for s in range(num_security):
                ...
                if security_queue_front_wait_remaining[s] <= 0:
                    ...
                    security_service_remaining[s] = get_current_security_service_time(sim_time_now)
                    sim.switch_agent_journey(
        ...
        # Make all waiting/queue spot markers blend into the background so
        # they are not visible on the rendered layout.
        waypoint_fill_color = background_color
        waypoint_border_color = background_color
    ...
    def _subsystem_active_slice(
        queue_lens: list[int], util: list[float]
    ) -> tuple[int, int] | None:
        """First/last index where the subsystem has queued work or servers busy."""
        n = len(queue_lens)
        if n == 0 or n != len(util):
            return None
        start: int | None = None
        end: int | None = None
        for i in range(n):
            if queue_lens[i] > 0 or util[i] > 0.0:
                if start is None:
                    start = i
                end = i
    ...
    ci_slice = _subsystem_active_slice(ci_queue_lengths, checkin_utilization_series)
    if ci_slice is not None:
        ci_lo, ci_hi = ci_slice
        ci_seg = ci_queue_lengths[ci_lo : ci_hi + 1]
        avg_ci_queue = sum(ci_seg) / len(ci_seg)
        peak_ci_queue = max(ci_seg)
        t_ci_start = time_steps[ci_lo]
        t_ci_end = time_steps[ci_hi]
    else:
        avg_ci_queue = 0.0
        peak_ci_queue = 0
        t_ci_start = 0.0
    ...
    sc_slice = _subsystem_active_slice(sc_queue_lengths, security_utilization_series)
    if sc_slice is not None:
        sc_lo, sc_hi = sc_slice
        sc_seg = sc_queue_lengths[sc_lo : sc_hi + 1]
        avg_sc_queue = sum(sc_seg) / len(sc_seg)
        peak_sc_queue = max(sc_seg)
        t_sc_start = time_steps[sc_lo]
        t_sc_end = time_steps[sc_hi]
    else:
        avg_sc_queue = 0.0
        peak_sc_queue = 0
        t_sc_start = 0.0
    ...
    if ci_slice is not None:
        ci_wait_samples = [
            ...
            and t_ci_start <= ci_service_start_times[aid] <= t_ci_end
        ]
    else:
        ci_wait_samples = []
    if sc_slice is not None:
        sc_wait_samples = [
            ...
            and t_sc_start <= sc_service_start_times[aid] <= t_sc_end
        ]
    else:
        sc_wait_samples = []
    ...
    scenario_prefix = "7_All_degarded_"

    def prefixed_name(filename: str) -> str:
        return f"{scenario_prefix}{filename}"

    metrics_path = os.path.abspath(os.path.join(_out_dir, prefixed_name("metrics.json")))
    timeseries_json_path = os.path.abspath(os.path.join(_out_dir, prefixed_name("timeseries.json")))
    timeseries_csv_path = os.path.abspath(os.path.join(_out_dir, prefixed_name("timeseries.csv")))
    plotdata_json_path = os.path.abspath(os.path.join(_out_dir, prefixed_name("plotdata.json")))
    passenger_llm_csv_path = os.path.abspath(os.path.join(_out_dir, prefixed_name("passenger_params.csv")))

    llm_param_summary = summarize_display_llm_params(passenger_display_llm_params)
    ...
    with open(passenger_llm_csv_path, "w", newline="", encoding="utf-8") as f:
        pp_fields = [
            "agent_id",
            "display_visit_time_s",
            "misroute_prob",
            "dwell_prob",
            "dwell_min_s",
            "dwell_max_s",
        ...
        pp_writer = csv.DictWriter(f, fieldnames=pp_fields)
        pp_writer.writeheader()
        for aid in sorted(passenger_display_llm_params.keys()):
            rec = passenger_display_llm_params[aid]
            pp_writer.writerow(
                {
                    "agent_id": aid,
                    "display_visit_time_s": rec["display_visit_time_s"],
                    "misroute_prob": rec["misroute_prob"],
                    "dwell_prob": rec["dwell_prob"],
                    "dwell_min_s": rec["dwell_min_s"],
                    "dwell_max_s": rec["dwell_max_s"],
                    "llm_source": rec["llm_source"],
                }
            )
    ...
    try:
        ...
        sec_per_hour = 3600.0
        disruption_marker_seconds = 20_000.0
        disruption_marker_hours = disruption_marker_seconds / sec_per_hour

        def add_disruption_marker(ax: plt.Axes) -> None:
            ax.axvline(
                disruption_marker_hours,
                color="0.35",
                linestyle=":",
                linewidth=1.4,
            )
    ...
    print(
        ...
        f"(check-in {NORMAL_CI_SERVICE_TIME:.0f}s -> {DEGRADED_CI_SERVICE_TIME:.0f}s and "
        f"security {NORMAL_SECURITY_SERVICE_TIME:.0f}s -> {DEGRADED_SECURITY_SERVICE_TIME:.0f}s "
        f"at/after {DEGRADATION_START_TIME:.0f}s; "
        f"display LLM disruption for display visits at/after {DISPLAY_LLM_DISRUPTION_START_S:.0f}s)",
        flush=True,
    )
    print(
        "Display LLM passenger parameter summary:",
        f"affected={llm_param_summary['llm_display_affected_count']},",
        f"misroute_prob min/max="
        f"{llm_param_summary['misroute_prob_generated_min']}/"
        f"{llm_param_summary['misroute_prob_generated_max']},",
        f"dwell_prob min/max="
        f"{llm_param_summary['dwell_prob_generated_min']}/"
        f"{llm_param_summary['dwell_prob_generated_max']},",
        f"dwell_min_s min/max="
        f"{llm_param_summary['dwell_min_s_generated_min']}/"
        f"{llm_param_summary['dwell_min_s_generated_max']},",
        f"dwell_max_s min/max="
        f"{llm_param_summary['dwell_max_s_generated_min']}/"
        f"{llm_param_summary['dwell_max_s_generated_max']}",
        flush=True,
    )
    ...
    print("CSV:", passenger_llm_csv_path)
